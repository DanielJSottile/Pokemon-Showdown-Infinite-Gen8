...
let BattleAbilities = {
	...
	"arenatrap": {
		desc: "For the first turn after this Pokemon switches in, prevents adjacent opposing Pokemon from choosing to switch out unless they are immune to trapping or are airborne.",
		shortDesc: "Prevents adjacent foes from choosing to switch unless they are airborne for 1 turn.",
		onStart(pokemon) {
			pokemon.addVolatile('arenatrap');
		},
		onEnd(pokemon) {
			delete pokemon.volatiles['arenatrap'];
			this.add('-end', pokemon, 'Arena Trap', '[silent]');
		},
		effect: {
			duration: 2,
			...
			onEnd(pokemon) {
				this.add('-end', pokemon, 'Arena Trap');
			},
		},
		onBeforeMovePriority: 15,
		onBeforeMove(pokemon) {
			pokemon.removeVolatile('arenatrap');
			this.add('-end', pokemon, 'Arena Trap', '[silent]');
		},
		onFoeTrapPokemon(pokemon) {},
		id: "arenatrap",
	...
	"battlebond": {
		...
		onSourceFaint(target, source, effect) {
			...
			if (effect && effect.effectType === 'Move' && source.template.speciesid === 'delphox' && source.hp && !source.transformed && source.side.foe.pokemonLeft) {
				this.add('-activate', source, 'ability: Battle Bond');
				source.formeChange('Delphox-Serena', this.effect, true);
			}
			if (effect && effect.effectType === 'Move' && source.template.speciesid === 'chesnaught' && source.hp && !source.transformed && source.side.foe.pokemonLeft) {
				this.add('-activate', source, 'ability: Battle Bond');
				source.formeChange('Chesnaught-Clemont', this.effect, true);
			}
		},
		onModifyMovePriority: -1,
	...
	"cottondown": {
		desc: "When the Pok�mon is hit by an attack, it scatters cotton fluff around and lowers the Speed stat of all Pok�mon except itself.",
		shortDesc: "Lowers Speed of all Pok�mon except itself when hit by an attack.",
	...
	"flowergift": {
		desc: "If this Pokemon is a Cherrim and Sunny Day is active, it changes to Sunshine Form and the Attack and Special Defense of it and its allies are multiplied by 1.5. If this Pokemon a Cherrim and it is holding Utility Umbrella, it remains in its regular form and the Attack and Special Defense stats of it and its allies are not boosted. If an ally is holding Utility Umbrella while Cherrim is in its Sunshine Form, they will not receive the Attack and Special Defense boosts.",
		shortDesc: "If user is Cherrim and Sunny Day is active, it transforms and allies' Attack and Sp. Def are 1.5x.",
		onStart(pokemon) {
	...
	"forecast": {
		...
		onUpdate(pokemon) {
			...
			case 'sandstorm':
				...
				break;
			case 'maelstrom':
				if (pokemon.template.speciesid !== 'castformnasty') forme = 'Castform-Nasty';
				break;
			default:
	...
	"furcoat": {
		shortDesc: "This Pokemon's Defense is multiplied by 1.8x.",
	...
	"hugepower": {
		shortDesc: "This Pokemon's Attack is multpilied by 1.8.",
		onModifyAtkPriority: 5,
		onModifyAtk(atk) {
			return this.chainModify(1.8);
		},
	...
	"hungerswitch": {
		desc: "The Pok�mon changes its form, alternating between its Full Belly Mode and Hangry Mode after the end of each turn.",
	...
	"libero": {
		shortDesc: "Changes the Pok�mon's type to the type of the move it's about to use.",
	...
	"magnetpull": {
		desc: "For the first turn after this Pokemon switches in, prevents adjacent opposing Steel-type Pokemon from choosing to switch out unless they are immune to trapping.",
		shortDesc: "Prevents adjacent Steel-type foes from choosing to switch for 1 turn.",
		onStart(pokemon) {
			pokemon.addVolatile('magnetpull');
		},
		onEnd(pokemon) {
			delete pokemon.volatiles['magnetpull'];
			this.add('-end', pokemon, 'Magnet Pull', '[silent]');
		},
		effect: {
			duration: 2,
			onFoeTrapPokemon(pokemon) {
			...
			onEnd(pokemon) {
				this.add('-end', pokemon, 'Magnet Pull');
			},
		},
		onBeforeMovePriority: 15,
		onBeforeMove(pokemon) {
			pokemon.removeVolatile('magnetpull');
			this.add('-end', pokemon, 'Magnet Pull', '[silent]');
		},
		onFoeTrapPokemon(pokemon) {},
		id: "magnetpull",
	...
	"mimicry": {
		shortDesc: "Changes the Pok�mon's type depending on the terrain.",
		onStart(pokemon) {
	...
	"neutralizinggas": {
		desc: "If the Pok�mon with Neutralizing Gas is in the battle, the effects of opposing Pok�mon's Abilities will be nullified or will not be triggered.",
	...
	"owntempo": {
		shortDesc: "This Pokemon cannot be confused or Intimidated. Gaining this Ability while confused cures it.",
		onUpdate(pokemon) {
	...
	"parentalbond": {
		...
		onDamagePriority: 1,
		onDamage(basePower, pokemon, target, move) {
			if (move.id === 'seismictoss' && move.multihitType === 'parentalbond' && move.hit > 1) return damage / 4;
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, pokemon, target, move) {
			if (move.multihitType === 'parentalbond' && move.hit > 1) return this.chainModify(0.25);
	...
	"perishbody": {
		desc: "When hit by a move that makes direct contact, the Pok�mon and the attacker will faint after three turns unless they switch out of battle.",
		shortDesc: "When hit by a contact move, the Pok�mon and the attacker faint in 3 turns.",
		onAfterDamage(damage, target, source, move) {
	...
	"punkrock": {
		desc: "Boosts the power of sound-based moves. The Pok�mon also takes half the damage from these kinds of moves.",
	...
	"rattled": {
		...
		shortDesc: "This Pokemon's Speed is raised 1 stage if hit by a Bug-, Dark-, or Ghost-type attack, OR Intimidate.",
		onAfterDamage(damage, target, source, effect) {
	...
	"screencleaner": {
		desc: "On switch-in, this Pok�mon ends the effects of Reflect, Light Screen, and Aurora Veil for both the user's and the opposing side.",
	...
	"serenegrace": {
		shortDesc: "This Pokemon's moves have their secondary effect chance multiplied by 1.5.",
		onModifyMovePriority: -2,
		onModifyMove(move) {
			if (move.secondaries) {
				this.debug('one and one half secondary chance');
	...
	"sheerforce": {
		desc: "This Pokemon's attacks with secondary effects have their power multiplied by 1.2, but the secondary effects are removed.",
	...
	"unburden": {
		desc: "If this Pokemon loses its held item for any reason, its Speed is multiplied by 1.8x. This boost is lost if it switches out or gains a new item or Ability.",
		shortDesc: "Speed is multiplied by 1.8x on held item loss; boost is lost if it switches, gets new item/Ability.",
	...
	"wanderingspirit": {
		desc: "The Pok�mon exchanges Abilities with a Pok�mon that hits it with a move that makes direct contact.",
	...
	"waterbubble": {
		desc: "This Pokemon's attacking stat is multiplied by 1.8x while using a Water-type attack. If a Pokemon uses a Fire-type attack against this Pokemon, that Pokemon's attacking stat is halved when calculating the damage to this Pokemon. This Pokemon cannot be burned. Gaining this Ability while burned cures it.",

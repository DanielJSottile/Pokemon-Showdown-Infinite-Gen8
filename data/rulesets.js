...
let BattleFormats = {
	...
	infinitetimer: {
		effectType: 'Rule',
		name: 'Infinite Timer',
		desc: "Pokemon Infinite's timer: 90 second Team Preview, 60 minutes Your Time, 45 seconds per turn",
		timer: {starting: 60 * 60, addPerTurn: 0, maxPerTurn: 40, maxFirstTurn: 90, grace: 90, timeoutAutoChoose: true, dcTimerBank: false},
	},
	speciesclause: {
		effectType: 'ValidatorRule',
		name: 'Species Clause',
	...
	eternatuseternamaxclause: {
		effectType: 'Rule',
		name: 'Eternatus Eternamax Clause',
		desc: "Prevents Eternamax from Eternamaxing",
		onBegin() {
			this.add('rule', 'Eternatus Eternamax Clause: You cannot Eternamax Eternatus');
			for (const pokemon of this.getAllPokemon()) {
				if (pokemon.speciesid === 'eternatus') pokemon.canGigantamax = null;
			}
		},
	},
	dynamaxclause: {

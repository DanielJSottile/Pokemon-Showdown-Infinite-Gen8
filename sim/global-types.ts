 ...
 * all - The move targets the field or all Pok�mon at once.
 * allAdjacent - The move is a spread move that also hits the user's ally.
 * allAdjacentFoes - The move is a spread move.
 * allies - The move affects all active Pok�mon on the user's team.
 * allySide - The move adds a side condition on the user's side.
 * allyTeam - The move affects all unfainted Pok�mon on the user's team.
 * any - The move can hit any other active Pok�mon, not just those adjacent.
 * foeSide - The move adds a side condition on the foe's side.
 * normal - The move can hit one adjacent Pok�mon of your choice.
...
interface BattleScriptsData {
	...
	canTimeTravel?: (this: Battle, pokemon: Pokemon) => string | undefined | null
...
interface ModdedBattlePokemon {
	...
	getRequestData?: (this: Pokemon) => {moves: {move: string, id: ID, target?: string, disabled?: boolean}[], maybeDisabled?: boolean, trapped?: boolean, maybeTrapped?: boolean, canMegaEvo?: boolean, canUltraBurst?: boolean, canTimeTravel?: boolean, canZMove?: ZMoveOptions}
...
namespace Actions {
	...
	export interface MoveAction {
		...
		/** a move to use (move action only) */
		move: Move;
		/** true if megaing, ultra bursting, or time traveling */
		mega: boolean | 'done';

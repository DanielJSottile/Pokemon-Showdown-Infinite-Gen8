...
export class TeamValidator {
	...
	validateStats(set: PokemonSet, template: Template, setSources: PokemonSources) {
		...
		for (const moveName of set.moves) {
			...
			if ((move.id === 'hiddenpower' || move.id === 'innerpower') && move.type !== 'Normal') {
				if (!set.hpType) {
					set.hpType = move.type;
				} else if (set.hpType !== move.type && ruleTable.has('obtainablemisc')) {
					problems.push(`${name}'s Hidden/Inner Power type ${set.hpType} is incompatible with Hidden/Inner Power ${move.type}`);
				}
			}
		}
